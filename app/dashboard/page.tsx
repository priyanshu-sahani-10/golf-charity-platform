...
export default function DashboardPage() {
  ...
  const fetchWinnings = async (uid: string) => {
    const { data } = await supabase
      .from("winners")
      .select(
        `id, match_count, prize_amount, payment_status, created_at, draws (created_at)`,
      )
      .eq("user_id", uid)
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-5 sm:p-6">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-full bg-[#4a9850]/20 border border-[#4a9850]/30 flex items-center justify-center text-2xl">
              👤
            </div>
            ...
            <div>
              ...
              <p className="text-white/50 text-sm mt-1">{fullName}</p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mt-5">
            <div className="rounded-xl bg-white/5 border border-white/10 p-3">
              ...
              <p className="text-[#4a9850] font-semibold mt-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {subscription ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[#4a9850] font-semibold text-sm uppercase tracking-wider">
                    Active
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {[
...

                    {
                      label: "Expires On",
                      value: new Date(
                        new Date(subscription.created_at).setMonth(
              ...
              <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <p className="text-[#d4af37]/70 text-xs uppercase tracking-widest">
                          Yearly
                        </p>
                ...
                <p className="text-white/25 text-xs">
                  20% of your subscription goes directly to your chosen charity.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-5 sm:p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              {scores.length === 0 ? (
                <p className="text-white/30 text-sm py-3 text-center">
                ...
                <div className="grid grid-cols-5 gap-2">
                  {scores.map((s, i) => (
                    <div
                      key={s.id}
                      className="rounded-xl bg-white/5 border border-white/10 p-3 flex flex-col items-center gap-1"
                    >
                      <span className="text-white/30 text-[10px] uppercase tracking-widest">
                        #{i + 1}
                      </span>
                      <span
                        className={`text-2xl font-black ${getScoreColor(s.score)}`}
                      >
                        {s.score}
                      </span>
                    </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm p-5 sm:p-6">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            ...
            <div className="rounded-xl bg-white/5 border border-[#4a9850]/20 p-4">
              <p className="text-white/40 text-xs uppercase tracking-widest mb-2">
                Charity Donation (20%)
              </p>
              <p className="text-3xl font-black text-[#4a9850]">
                ₹{subscription ? (subscription.amount * 0.2).toFixed(2) : "0"}
              </p>
            </div>

            <div className="rounded-xl bg-white/5 border border-[#d4af37]/20 p-4">
              <p className="text-white/40 text-xs uppercase tracking-widest mb-2">
                Prize Pool (80%)
              </p>
              <p className="text-3xl font-black text-[#d4af37]">
                ₹{subscription ? (subscription.amount * 0.8).toFixed(2) : "0"}
              </p>
            </div>
          </div>
        </div>
                  ...
                  {winnings.map((win) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-3">
                        <span
                          className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${getStatusStyle(win.payment_status)}`}
                        >
                          {win.payment_status}
                        </span>
                      </td>
